...
for element in root_child_elements:
    ...
    if tag == 'NOTE':
        ...
        if value.startswith('url:'):
            value = value[len("url: "):]
            note_url_dict[pointer] = value

# Create dictoionary mapping source identifier to openarchieven.nl url.
sour_url_dict = {}
for element in root_child_elements:
    # Handle SOUR
    if isinstance(element, SourceElement):
        ...
        for child_element in child_elements:
            ...
            if tag == "NOTE":
                ...
                url = note_url_dict[note_pointer] 
                if "openarchieven.nl" in url:
                    sour_url_dict[sour_pointer] = url

# resolving and relating source
oa_source_uri = None
for sour, url in sour_url_dict.items():

    request_url = url + "/ttl:pico"
    g = Graph()
    g.parse(request_url)
